...
export const CreateFarmSchema = z.object({
 //Step 1 fields
  ...
  address: z.string().min(10, 'Address must be at least 10 characters').max(255),
  ...
  capacity: z.number().min(100, 'Minimum capacity is 100 birds').max(1000000, "Maximun capacity exceeded"),
  housingSystem: z.nativeEnum(HousingSystem, {required_error: "Select housing system"}),
  certifications: z
    .array(z.string())
    ...
    longitude: z.number().min(-180, 'Invalid latitude').max(180, 'Invalid latitude').optional(),
    media: z
    .array(
      z.object({
        url: z.string(),
