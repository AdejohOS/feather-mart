...
import { CreateFarmSchema, CreateFarmValues, CreateProductSchema, UpdateFarmSchema, UpdateFarmValues, UpdateProfileSchema } from "./vendor/vendor-schema"
import { revalidatePath } from "next/cache"
import { redirect } from "next/navigation"
import { HousingSystem, PoultryType } from "@/types/types"
import { deleteProductMedia, saveProductMedia, uploadProductMedia } from "@/lib/upload-product"
...
export const updateProfileAction = async (prevState: {success: boolean, error: string | null},  formData: FormData) => {
  ...
  if (userError || !user) {
    ...
    const {error} = await supabase.from("profiles").update(validatedData.data
    ).eq("id", user.id)
...
if ( profileError || !profile || profile?.role !== 'seller' ) {
  ...
  const farmData = {
    ...
    location_geo: validatedData.data.latitude && validatedData.data.longitude ? `SRID=4326;POINT(${validatedData.data.longitude} ${validatedData.data.latitude})`
    : null,
  ...
  const {data: existingFarm} = await supabase.from("farms").select("id").eq("name", validatedData.data.name).eq("seller_id", user.id).single()
...
export async function updateFarmAction(data: UpdateFarmValues, farmId: string) {
  ...
  try {
    ...
    const {data:existingFarm, error:existingFarmError} =await supabase.from("farms").select("*").eq("id", farmId).single()
    ...
    const mediaToDelete = oldMedia.filter(oldMedia => !newMedia.some(newItem=> newItem.path === oldMedia.path)).map(item=> item.path)
  
    if (mediaToDelete.length>0) {
      const {error:deleteError} = await supabase.storage.from("farm_media").remove(mediaToDelete)
    ...
    const {error: updateError} = await supabase.from("farms").update(farmData).eq("id", farmId)
 ...
 export async function deleteFarmAction(farmId: string){
  ...
  try {
    ...
    const {data: existingFarm, error: existingFarmError} = await supabase.from("farms").select("id, seller_id, media").eq("id", farmId).single()
    ...
    if(existingFarm.media && Array.isArray(existingFarm.media) ){
      const mediaPaths = (existingFarm.media as Array<{path:string}>).filter(item => item?.path).map(item=> item.path)

      if (mediaPaths.length > 0) {
        const {error:storageError} = await supabase.storage.from("farm_media").remove(mediaPaths)
    ...
    const {error: deleteError} = await supabase.from("farms").delete().eq("id", farmId)
 ...
 export async function createProductAction(prevState: FormState, formData: FormData): Promise<FormState>{
  ...
  const booleanFields = ["isOrganic", "isFreeRange", "isAntibiotic", "isHormone", "isVaccinated", "isAvailable"]
  ...
  const productData = {
    seller_id: user.id, 
      ...
      available_date: validatedData.data.availableDate ? new Date(validatedData.data.availableDate).toISOString().split("T")[0] : null,
      ...
      tags: Array.isArray(validatedData.data.tags) ? validatedData.data.tags : [],
 ...
 export async function deleteProductAction (productId:string){
  ...
  try {
    ...
    const {data:existingProduct, error:existingProductError}= await supabase.from("products").select("id, seller_id").eq("id", productId).eq("seller_id", user.id).single()
...
// Delete media files
const {data:mediaItems, error:mediaError} = await supabase.from("product_media").select("*").eq("product_id", productId)

if(!mediaItems || mediaError){
  return {
    success: false,
...
if (mediaItems && mediaItems.length > 0) {
  ...
  if (filePaths.length > 0) {
    const { error: storageError } = await supabase.storage.from("product-media").remove(filePaths)
 ...
 export async function deleteMediaAction(mediaId: string){
  ...
  try {
    ...
    const {data:existingMedia, error:existingMediaError} = await supabase.from("product_media").select("product_id, id").eq("id", mediaId).single()
  if (!existingMedia || existingMediaError){
    return {success: false, message: `Error fetching media ${existingMediaError.message}`}
  }

  const {data: product,error:fetchProductError} = await supabase.from("products").select("seller_id").eq("id", existingMedia.product_id).single()
  if (fetchProductError) {
    return { success: false, message: `Error fetching product: ${fetchProductError.message}` }
  }

  if (product.seller_id !== user.id) {
    return { success: false, message: "You can only delete media for your own products" }
  }
 ...
 export async function getSellerFarms(){
  const supabase = await createClient()
