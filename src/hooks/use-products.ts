 ...
 export const useUpdateProduct = () => {
    ...
    return useMutation({
      mutationFn: async (formData: ProductFormData) => {
        ...
        if (formData.uploadedMedia && formData.uploadedMedia.length > 0) {
          // Filter out media that already has an ID (existing media)
          const newMedia = formData.uploadedMedia.filter((media) => !media.id)

          if (newMedia.length > 0) {
            const mediaEntries = newMedia.map((media) => ({
