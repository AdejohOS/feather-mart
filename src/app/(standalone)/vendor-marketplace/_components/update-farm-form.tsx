...
import { FarmTypes, HousingSystem, PoultryType } from '@/types/types'
...
import { createFarmAction, updateFarmAction } from '@/features/action'
import { useRouter } from 'next/navigation'
import { FileUpload } from './file-upload'
import { useQueryClient } from '@tanstack/react-query'

interface UpdateFarmFormProps {
  initialValues: FarmTypes
  farmId: string
}

export const UpdateFarmForm = ({
  initialValues,
  farmId
}: UpdateFarmFormProps) => {
  ...
  const form = useForm<CreateFarmValues>({
    ...
    defaultValues: {
      ...
      farmEmail: initialValues?.farm_email || '',
      poultryType:
        (initialValues?.poultry_type as PoultryType) || PoultryType.Layers,
      capacity: initialValues?.capacity,
      housingSystem:
        (initialValues?.housing_system as HousingSystem) ||
        HousingSystem.BatteryCages,
      certifications: initialValues?.certifications ?? [],
      media:
        (initialValues?.media as {
          name?: string
          path?: string
          type?: string
          url?: string
  ...
  const onSubmit = async (data: UpdateFarmValues) => {
    console.log('Values', data.state)
    startTransition(async () => {
      const result = await updateFarmAction(data, farmId)
      ...
      if (result?.error) {
        ...
        toast.success('Farm records updated successfully!')
      }
    })
  }
  ...
  return (
    <Card>
      <CardHeader className='text-center'>
        <CardTitle>Update farm records</CardTitle>
        <CardDescription>Fill out form to update farm record.</CardDescription>
      </CardHeader>
      <div className='px-7'>
        <DottedSeparator />
      </div>
