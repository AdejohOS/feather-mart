...
import { createFarmAction, updateFarmAction } from '../action'
  ...
  async function onSubmit(data: FarmFormValues) {
    try {
      ...
      if (isEditing && initialData?.id) {
        ...
        const result = await createFarm.mutateAsync(formData)
        router.push(`/vendor-marketplace/settings`)
      }
    } catch (error: any) {
                ...
                <div className='grid gap-6 md:grid-cols-2'>
                  ...
                  <div className='md:col-span-2'>
                    <MediaUpload
                      form={form}
                      maxFiles={10}
                      existingMedia={form.getValues('existingMedia')}
                      mediaType='farm'
                    />
                  </div>
                </div>
