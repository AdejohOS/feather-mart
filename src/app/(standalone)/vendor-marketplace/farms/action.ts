...
export async function createFarmAction(
  ...
  try {
    ...
    if (typeof formDataObject.establishedDate === "string") {
    ...
    const farmData = {
      ...
      established_date: validatedData.data.establishedDate
        ? validatedData.data.establishedDate.toISOString().split("T")[0]
        : null,
    ...
    if (validatedData.data.uploadedMedia) {
      try {
        ...
        if (
          ...
          console.log(
            `Inserting ${uploadedMedia.length} media entries for farm ${farmId}`
          );
    ...
    if (validatedData.data.uploadedMedia) {
      try {
        ...
        if (
          ...
          console.log(
            `Updating: Inserting ${uploadedMedia.length} media entries for farm ${farmId}`
          );
...
export async function deleteFarmAction(farmId: string) {
  ...
  try {
    ...
    const { data: sellerFarm, error: sellerFarmError } = await supabase
      .from("farms")
      .select("seller_id")
      .eq("id", farmId)
      .single();
...
export async function deleteFarmMediaAction(mediaId: string) {
  ...
  try {
    ...
    if (fetchFarmError) {
      return {
        success: false,
        message: `Error fetching farm: ${fetchFarmError.message}`,
      };
    }
    ...
    if (farm.seller_id !== user.id) {
      return {
        success: false,
        message: "You can only delete media for your own farms",
      };
    }
    ...
    if (dbError) {
      return {
        success: false,
        message: `Error deleting media: ${dbError.message}`,
      };
    }
