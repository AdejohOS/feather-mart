 ...
 */
export async function uploadProductMedia(productId: string, files: File[]): Promise<UploadedFile[]> {
 ...
 */
export async function saveProductMedia(productId: string, files: UploadedFile[]): Promise<void> {
  const supabase = await createClient()
  if (!files.length) return
...
export async function deleteProductMedia(mediaId: string): Promise<boolean> {
  ...
  try {
    ...
    const { data, error: fetchError } = await supabase.from("product_media").select("url").eq("id", mediaId).single()
